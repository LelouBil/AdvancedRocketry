...
public class TilePressureEqualizer extends TileOxygenSystemBase implements IComparatorOverride, IModularInventory {

    public TilePressureEqualizer() {
        super(AdvancedRocketryTileEntityType.TILE_PRESSURE_EQUALIZER);
    }

    public boolean canPerformFunction(AtmosphereHandler handler, World world, int remainingPower, float currentTotalPressure) {
        return currentTotalPressure > 100 && remainingPower >= 50 && (tank.getFluid().isEmpty() || tank.getCapacity() - 100 >= tank.getFluid().getAmount());
    }

    public int performFunction(AtmosphereHandler handler, World world, int addedVolumeThisTick) {
        Fluid producedFluid = DimensionManager.getInstance().getDimensionProperties(world).hasOxygen ? AdvancedRocketryFluids.spentAirStill.get() : AdvancedRocketryFluids.oxygenlessAirStill.get();
        return tank.fill(new FluidStack(producedFluid, 50), FluidAction.EXECUTE);
    }

    @Override
    public String getModularInventoryName() {
        return "block.advancedrocketry.pressureequalizer";
    }
